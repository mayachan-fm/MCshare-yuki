...
// ==============================================
// MUAT DATA AWAL (SUDAH DIPERBAIKI)
// ==============================================
async function tampilkanAddon() {
    const wadah = document.getElementById('wadah-addon');
    try {
        // Ganti jadi ./data.json agar alamat file lebih pasti
        ...
        console.log("✅ Data berhasil dimuat:", semuaDataAddon); // Cek di konsol jika perlu
        ...
        const totalUnduh = semuaDataAddon.reduce((jml, item) => jml + (item['jumlah unduh'] || 0), 0);
...
function tampilkanTopAddon() {
    ...
    if (!wadah || semuaDataAddon.length === 0) return;

    const terurut = [...semuaDataAddon].sort((a,b) => (b['jumlah unduh']||0) - (a['jumlah unduh']||0)).slice(0, 8);
    
    wadah.innerHTML = '';
    terurut.forEach((item) => {
        const indeksAsli = semuaDataAddon.indexOf(item);
        ...
        `;
        kartu.addEventListener('click', () => window.location.href = `detail.html?id=${indeksAsli}`);
...
function tampilkanDaftar(dataYangDitampilkan) {
    ...
    dataYangDitampilkan.forEach((item, indeks) => {
        ...
        const sudahDiLike = JSON.parse(localStorage.getItem('sudahLike') || '[]').includes(indeks.toString());
            ...
            <div class="kartu-isi">
                ...
                <div class="bagian-aksi">
                    ...
                    <div class="grup-tombol-kanan">
                        <button class="tombol-like ${sudahDiLike ? 'sudah' : ''}" data-id="${indeks}"><i class="fa fa-thumbs-up"></i> ${jumlahLike}</button>
...
function terapkanFilterDanCari() {
    ...
    const hasil = semuaDataAddon.filter(item => {
